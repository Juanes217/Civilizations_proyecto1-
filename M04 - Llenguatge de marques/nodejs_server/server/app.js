...
app.get('/civilitzacio', async (req, res) => {
    try {
        ...
        res.render('civilitzacio', { stats: { fusta: 0, ferro: 0, menjar: 0, mana: 0 }, page: 'civilitzacio' });
    }
});
...
app.get('/batalles', async (req, res) => {
    try {
        ...
        res.render('batalles', { batalles: rows, total: rows.length, page: 'historial' }); 
    } catch (error) {
...
// 5. EQUIP / PROGRAMADORS
app.get('/programadors', (req, res) => {
    res.render('programadors', { page: 'programadors' });
});


// ================= CONFIGURACIÓN ADAPTABLE PARA LOCAL Y PROXMOX =================
